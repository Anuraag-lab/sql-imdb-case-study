...
SELECT COUNT(*) AS "No.Of rows director_mapping" FROM director_mapping;
SELECT COUNT(*) AS "No.Of genre" FROM genre;
SELECT COUNT(*) AS "No.Of rows movie" FROM movie;
SELECT COUNT(*) AS "No.Of rows names" FROM names;
SELECT COUNT(*) AS "No.Of rows ratings" FROM ratings;
SELECT COUNT(*) AS "No.Of rows role_mapping" FROM role_mapping;

-- NUMBER OF ROWS PER TABLE ARE 3867, 14662 , 7997 , 25735 , 7997 AND 15615 RESPECTIVELY

-- Q2. Which columns in the movie table have null values?
-- Type your code below:
SELECT 
SUM(CASE WHEN id IS NULL THEN 1 ELSE 0 END) AS "ID NULL VALUES",
SUM(CASE WHEN title IS NULL THEN 1 ELSE 0 END) AS "TITLE NULL VALUES",
SUM(CASE WHEN year IS NULL THEN 1 ELSE 0 END) AS "YEAR NULL VALUES",
SUM(CASE WHEN date_published IS NULL THEN 1 ELSE 0 END) AS "DATE_PUBLISHED NULL VALUES",
SUM(CASE WHEN duration IS NULL THEN 1 ELSE 0 END) AS "DURATION NULL VALUES",
SUM(CASE WHEN country IS NULL THEN 1 ELSE 0 END) AS "COUNTRY NULL VALUES",
SUM(CASE WHEN worlwide_gross_income IS NULL THEN 1 ELSE 0 END) AS "WORLDWIDE GROSS INCOME NULL VALUES",
SUM(CASE WHEN languages IS NULL THEN 1 ELSE 0 END) AS "LANGUAGES NULL VALUES",
SUM(CASE WHEN production_company IS NULL THEN 1 ELSE 0 END) AS "PRODUCTION COMPANY NULL VALUES"
FROM movie;
...
-- THIS QUERY GIVES COUNT OF MOVIES WITH INDIA AND USA AND INCLUDES OVERALL WHERE THEY ARE PRESENT 
SELECT 
...
-- Drama movies were produced highest overall accross the globe 4285 across all years.
...
-- thriller movies rank 3rd , 1st position is held by drama and 2nd by comedy.
...
-- Dream Warrior Pictures and National theatre live are production companies which have highest hit movies produced. 
...
SELECT
	count(movie_id) AS movies_apr18_apr19,
	median_rating
FROM ratings AS r
INNER JOIN movie AS m 
ON r.movie_id = m.id
...
SELECT 
	...
	SUM(CASE WHEN height IS NULL THEN 1 ELSE 0 END) AS heaight_null,
...
-- top 2 top performing actors are Mammotty and Mohanlal :they have done maximum number of movies among all 
-- and have a median rating greater then 8.
...
-- Marvel studios has highest number of votes, followed by twentieth centurey fox and warner bros.
...
-- Taapsee Panu is the highest rated actress 
...
-- Joker is a superhit movie,getout and split are hit movies are hit movies, these categorize as top movies based on votes recieved.
...
-- Top 3 Genres based on most number of movies
 ...
 dense_rank()  over(order by count(g.movie_id) desc) as movie_rank 
 from genre g 
...
FROM movie AS m 
INNER JOIN genre g ON m.id = g.movie_id
WHERE g.genre IN (SELECT g.genre FROM top_genre)
),
ranked_movies AS 
(
SELECT genre,
...
SELECT 
    ...
    COUNT(rm.movie_id) AS movie_count, 
    ROUND(SUM(total_votes * r.avg_rating) / SUM(total_votes), 2) AS actress_avg_rating, 
...
INNER JOIN movie m ON m.id = r.movie_id
INNER JOIN avg_diff a ON a.name_id = d.name_id 
GROUP BY d.name_id
